---
name: "Bran witnesses Jaime and Cersei"
type: event.incident
slug: bran-witnesses-jaime-and-cersei
aliases: ["Bran witnesses Jaime and Cersei", "Bran sees Jaime and Cersei", "Bran catches the Lannister twins", "Bran catches Jaime and Cersei", "bran-catches-the-lannister-twins", "bran-sees-jaime-and-cersei"]
confidence: tier-1
era: war-of-the-five-kings-prelude
containers: [bran]
pass_origin: s105-causal-track
node_version: 1
evidence_chapters:
  - AGOT Bran II
occurred:
  ac_year: 298
  precision: year
  basis_source: book-chapter
  basis_reliability: primary-canon
  date_confidence: tier-2
sort_keys:
  ac_year: 298
  book_order: 1
  chapter_number: 9
  chapter_label: "AGOT Bran II"
  composite: "0298.1.009"
  reading_order: "1.009"
  basis: "year+chapter"
---

## Identity

While climbing the abandoned First Keep at Winterfell during King Robert's visit, Bran Stark stumbles on Queen Cersei Lannister and her twin brother Ser Jaime coupling in a tower room. Bran recognizes them both. The discovery is the spark of the whole chain: it is *because* Bran has seen them that Jaime pushes him from the window moments later.

## Edges

(causal edge wired by the S105 causal-edges track — TRIGGERS [Jaime pushes Bran from the tower](jaime-pushes-bran-from-the-tower))

## Quotes

> Bran saw her face. Her eyes were closed and her mouth was open, moaning. Her golden hair swung from side to side as her head moved back and forth, but still he recognized the queen.

— AGOT Bran II (`sources/chapters/agot/agot-bran-02.md:135`)
