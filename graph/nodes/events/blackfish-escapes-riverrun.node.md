---
name: "The Blackfish escapes Riverrun"
type: event.incident
slug: blackfish-escapes-riverrun
aliases: ["the Blackfish escapes Riverrun", "Brynden Tully escapes Riverrun", "the Blackfish slips the siege"]
confidence: tier-1
era: war-of-the-five-kings
pass_origin: s159-jaime-riverlands-enrich
node_version: 1
evidence_chapters:
  - AFFC Jaime VII
occurred:
  ac_year: 300
  precision: year
  basis_source: book-chapter
  basis_reliability: pov-contemporaneous
sort_keys:
  ac_year: 300
  book_order: 4
  chapter_number: 45
  chapter_label: "AFFC Jaime VII"
  composite: "0300.4.045"
  reading_order: "4.045"
  basis: "year+chapter"
---

# The Blackfish escapes Riverrun

Rather than yield or take the black, [Ser Brynden Tully](brynden-tully) — the Blackfish — slips out
of Riverrun the night the castle changes hands. As [Edmure](edmure-tully) later confesses to
[Jaime](jaime-lannister), the garrison raised the portcullis on the Water Gate three feet underwater,
leaving a submerged gap while the gate still appeared closed; the Blackfish, a strong swimmer, pulled
himself beneath the spikes after dark and floated downriver beneath the boom — *"a black fish in a
black river floating quietly downstream."* In the confusion of the surrender it is the next morning
before Jaime learns Brynden is not among the prisoners. [Ser Addam Marbrand](addam-marbrand) leads the
search that never finds him; Jaime suspects the riverlords would sooner shelter him than catch him.

## Edges
(Edges in `graph/edges/edges.jsonl`, S159 Jaime/Riverlands enrichment. [Brynden Tully](brynden-tully)
AGENT_IN (he swims out); [Edmure](edmure-tully) AGENT_IN (he raised the Water Gate gap); ENABLES-in
from [Edmure's surrender](edmure-yields-riverrun); SUB_BEAT_OF [siege-of-riverrun](siege-of-riverrun);
LOCATED_AT [Riverrun](riverrun); [Addam Marbrand](addam-marbrand) PARTICIPATES_IN the hunt.
No SUSPECTED_OF edges: Jaime's thought that [Karyl Vance](karyl-vance)/[Clement Piper](clement-piper)
"and their ilk" would sooner abet it (affc-jaime-07:45) was adjudicated diffuse class-distrust, not
pointed suspicion — the proposed edges were REJECTED at S159 fresh-verify (E14/E15), reaffirmed S211.)

## Quotes

> "He did swim," said Edmure, sullenly.

> We raised the portcullis on the Water Gate. Not all the way, just three feet or so. Enough to leave a gap under the water, though the gate still appeared to be closed. My uncle is a strong swimmer. After dark, he pulled himself beneath the spikes.
