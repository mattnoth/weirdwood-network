---
name: House Merryweather
type: organization.house
slug: house-merryweather
aliases: []
confidence: tier-1
wiki_source: https://awoiaf.westeros.org/index.php/House_Merryweather
bucket_id: houses-reach-h
prompt_version: v1-python
node_version: 1
pass_origin: pass2-wiki
first_available:
  book: ASOS
  chapter: 6
  source: cite_ref
  pov: "Davos I"
sigil: A golden horn of plenty spilling out apples, carrots, plums, onions, leeks, turnips, and fruits of many colors on a white field bordered in gold (Argent, a cornucopia or overflowing with apples, carrots, plums, onions, leeks, turnips and fruits variegated all proper within a bordure of the second)
words: Behold Our Bounty
sigil_image: House_Merryweather.svg
---

## Identity

House Merryweather is a noble house from the Reach, seated at Longtable. Lord Orton Merryweather is the current head, with Russell Merryweather as heir. (wiki:House_Merryweather)

## Heraldry & Sigil

A golden horn of plenty spilling out apples, carrots, plums, onions, leeks, turnips, and fruits of many colors on a white field bordered in gold (Argent, a cornucopia or overflowing with apples, carrots, plums, onions, leeks, turnips and fruits variegated all proper within a bordure of the second) (wiki:House_Merryweather)

Words: "Behold Our Bounty" (wiki:House_Merryweather)

## Allegiances

House Merryweather is `SWORN_TO` House Tyrell as their overlord. (track_b: Overlord)

## Narrative Arc

Lord Orton Merryweather becomes a close ally of Cersei Lannister during AFFC, serving on the small council. His Myrish wife Taena Merryweather becomes one of Cersei's confidantes. The Merryweathers were previously exiled under the Mad King for supporting the Blackfyre pretenders but were restored. (wiki:House_Merryweather)

## Edges

- SEAT_OF (reverse): Longtable (track_b: Seat)
- RULES: Orton Merryweather (track_b: Head)
- REGION_OF: Reach (track_b: Region)
- HOLDS_TITLE: Lord of Longtable (track_b: Title)
- HEIR_TO: Russell Merryweather (track_b: Heir)
- OVERLORD_OF (reverse): House Tyrell (track_b: Overlord)

## Notes

Tier 2 (inference): The wiki implies previous exile and restoration under circumstances related to Blackfyre support, but the specific details require cross-referencing multiple sources.
