---
name: House Toland
type: organization.house
slug: house-toland
aliases: []
confidence: tier-1
wiki_source: https://awoiaf.westeros.org/index.php/House_Toland
bucket_id: houses-dorne
prompt_version: v1-python
node_version: 1
pass_origin: pass2-wiki
first_available:
  book: AFFC
  chapter: 13
  source: cite_ref
  pov: "Cersei III"
---

## Identity

House Toland is a lordly house from Dorne, seated at Ghost Hill (wiki:House_Toland).

## Allegiances

House Toland is `OVERLORD_OF` (reverse) House Martell (track_b: Overlord). Their seat is `SEAT_OF` Ghost Hill (track_b: Seat). Located in `REGION_OF` Dorne (track_b: Region). The current head is Lady Nymella Toland (track_b: Head). The heir is Valena Toland (track_b: Heir). The house holds the title `HOLDS_TITLE` Lord of Ghost Hill (track_b: Title).

## Appearances & Description

Their blazon is a green dragon biting its own tail on gold; formerly, a ghost (wiki:House_Toland). The change in sigil commemorates a brave fool who served as Lord Toland's champion against Aegon the Conqueror — the dragon biting its tail in the motley's colors of green and gold (wiki:House_Toland).

## Narrative Arc

During the First Dornish War, Lord Toland sent his mad fool as champion to face Aegon the Conqueror. After Aegon slew the man, he learned it was the Tolands' fool, while Lord Toland himself had escaped (wiki:House_Toland). The Dornishmen quickly recaptured Ghost Hill after Aegon departed. In 8 AC, Queen Visenya burned Ghost Hill from dragonback. During the Dragon's Wroth, Lady Toland was murdered for the Iron Throne's ransom on her head (wiki:House_Toland).

## Quotes

> "A dragon eating its own tail?" ... "The dragon is time. It has no beginning and no ending, so all things come round again." (affc-the-soiled-knight-01:269–271)
>
> —[Arys Oakheart](wiki:Arys_Oakheart) names the sigil ("A dragon eating its own tail?"); [Arianne Martell](wiki:Arianne_Martell) gives the meaning — on the arms of House Toland

## Edges

- SEAT_OF (reverse): Ghost Hill (track_b: Seat)
- RULES: Nymella Toland (track_b: Head)
- REGION_OF: Dorne (track_b: Region)
- HOLDS_TITLE: Lord of Ghost Hill (track_b: Title)
- HEIR_TO: Valena Toland (track_b: Heir)
- OVERLORD_OF (reverse): House Martell (track_b: Overlord)
