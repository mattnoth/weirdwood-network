---
name: House Wensington
type: organization.house
slug: house-wensington
aliases: []
confidence: tier-1
wiki_source: https://awoiaf.westeros.org/index.php/House_Wensington
bucket_id: houses-stormlands-h
prompt_version: v1-python
node_version: 1
pass_origin: pass2-wiki
first_available:
  book: ASOS
  chapter: 75
  source: cite_ref
---

## Identity

House Wensington is a noble house from the Stormlands. They blazon their arms with five gold trumpets on red, beneath a gold stag's head on a black chief. Their motto is "Sound The Charge." (wiki:House_Wensington)

## Allegiances

House Wensington is `REGION_OF` Stormlands and `OVERLORD_OF` (reverse) House Baratheon. (track_b: Region, Overlord)

## Narrative Arc

The current head is unknown. (track_b: Head — Unknown) Wensington banners are seen among those gathered at the Wall with Stannis Baratheon's forces. (wiki:House_Wensington.cite_ref-Rasos75)

## Edges

- RULES: Unknown (track_b: Head)
- REGION_OF: Stormlands (track_b: Region)
- OVERLORD_OF (reverse): House Baratheon (track_b: Overlord)
