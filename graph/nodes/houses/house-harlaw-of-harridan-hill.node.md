---
name: "House Harlaw of Harridan Hill"
type: organization.house
slug: house-harlaw-of-harridan-hill
aliases: []
confidence: tier-2
wiki_source: "https://awoiaf.westeros.org/index.php/House_Harlaw_of_Harridan_Hill"
bucket_id: tier3-pathb-orgs
prompt_version: v1-python
node_version: 1
pass_origin: pass2-wiki-deterministic
sigil: A silver scythe on pale blue (Bleu celeste, a scythe argent)
sigil_image: House_Harlaw_of_Harridan_Hill.svg
---

## Identity

House Harlaw of Harridan Hill is a noble house from the AWOIAF wiki.

## Heraldry & Sigil

A silver scythe on pale blue (Bleu celeste, a scythe argent) (wiki:House_Harlaw_of_Harridan_Hill)

## Edges

- SEAT_OF (reverse): Harridan Hill (track_b: Seat)
- SEAT_OF (reverse): Harlaw (track_b: Seat)
- RULES: Boremund Harlaw (track_b: Head)
- REGION_OF: Iron Islands (track_b: Region)
- HOLDS_TITLE: Master of Harridan Hill (track_b: Title)
- OVERLORD_OF (reverse): House Harlaw (track_b: Overlord)

## Narrative Arc

### A Feast for Crows[ ]

[Boremund Harlaw](wiki:Boremund_Harlaw) goes to [Ten Towers](wiki:Ten_Towers) to lend his support to [Asha Greyjoy](wiki:Asha_Greyjoy) in the upcoming [kingsmoot on Old Wyk](wiki:Kingsmoot_on_Old_Wyk).(wiki:House_Harlaw_of_Harridan_Hill.cite_ref-Raffc11.7B.7B.7B3.7D.7D.7D.7B.7B.7B4.7D.7D.7D_3-0)
