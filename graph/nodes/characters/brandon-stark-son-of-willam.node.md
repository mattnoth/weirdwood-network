---
name: "Brandon Stark (son of Willam)"
type: character.human
slug: brandon-stark-son-of-willam
aliases: []
confidence: tier-1
wiki_source: "https://awoiaf.westeros.org/index.php/Brandon_Stark_(son_of_Willam)"
bucket_id: characters-house-stark-b-h
prompt_version: v1
node_version: 2
pass_origin: pass2-wiki
first_available:
  book: AGOT
  chapter: 24
  source: cite_ref
  pov: "Daenerys III"
---

## Origins

Brandon's mother died in childbirth, so [Nan](wiki:Old_Nan) came to [Winterfell](wiki:Winterfell) to be his [wet nurse](wiki:Wet_nurse). He died at age three of a summer chill.(wiki:Brandon_Stark_(son_of_Willam).cite_ref-Ragot24.7B.7B.7B3.7D.7D.7D.7B.7B.7B4.7D.7D.7D_2-2)

## Edges

- PARENT_OF (reverse): Willam Stark → Brandon Stark (track_b: Father)
- PARENT_OF (reverse): Lyanne Glover → Brandon Stark (track_b: Mother)
- SWORN_TO: House Stark (track_b: Allegiance)
- CULTURE_OF: Northmen (track_b: Culture)
- BORN_AT: Winterfell (track_b: Born)
- DIED_AT: Winterfell (track_b: Died)

## Notes

Not to be confused with Brandon Stark, Eddard's elder brother. This Brandon is of an earlier generation — Eddard's great-uncle.
