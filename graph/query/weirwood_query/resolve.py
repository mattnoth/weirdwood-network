...
ZERO BEHAVIOR CHANGE (query-layer design.md step 1): this is a verbatim port
of the resolution algorithm and its scoring constants. In particular:
  - MIN_FUZZY_SCORE / MAX_FUZZY_CANDIDATES are unchanged.
  - The known length-bias in `_fuzzy_candidates` (no candidate-length penalty,
    so a short query can score 1.0 against a long candidate phrase) is a
    documented, KNOWN issue (design.md G10) and is explicitly NOT fixed here
    — that is step 4c (resolver hardening), a later, separately-gated step.
  - PRIORITY_ORDER / collision handling is unchanged.
...
def _fuzzy_candidates(
    ...
    candidate. Requires score >= MIN_FUZZY_SCORE. Returns a list of {slug,
    score, match_type: 'fuzzy', node_category, ...} sorted by score
    descending, up to max_results items.

    KNOWN ISSUE (not fixed here, see module docstring / design.md G10): no
    candidate-length penalty, so a short query can score 1.0 against a much
    longer candidate phrase (candidate-length bias).
    """
