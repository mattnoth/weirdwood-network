...
- `2026-06-30` — House sigil image binaries (`scripts/wiki-fetch-sigils.py` → `working/wiki/sigils/`). Reason: the `action=parse` crawl saved page HTML plus each infobox sigil `<img>` filename but never the referenced image binaries; the heraldry images are needed to render house sigils in the graph/front-end. Matt-approved this turn. Targets one field (sigil image files); `cloudscraper`, throttled ~1.8s, writes to `working/wiki/sigils/` (not `sources/`). Smoke-tested 5/5 clean SVGs, then scaled to the ~369 node-mapped houses.
- `2026-06-30` — Major-character portrait images (`scripts/wiki-fetch-portraits.py` → `working/wiki/portraits/`). Reason: same crawl gap as sigils — character infobox portrait `<img>` filenames were saved but not the binaries; portraits are needed to render major character nodes in the front-end. Matt-approved this turn. Targets one field (character infobox portrait); `cloudscraper`, throttled ~1.8s, writes to `working/wiki/portraits/` (not `sources/`). Scope limited to "major" nodes (graph-degree-ranked + POV characters); smoke-tested before any bulk run.
