...
## AFFC SPINE PHASE COMPLETE (S114–S117) — NEXT = the ESSOS container

**All 4 AFFC smoke-test fumbles are now spine-built:** #1 Cersei's downfall (S114), #3 Brienne→Stoneheart (S115), #2 Kingsmoot→Euron (S116), **#4 Dorne/Myrcella (S117).**

**S117 SHIPPED** the AFFC #4 Dorne/Myrcella (Queenmaker) arc: 2 new beats (`the-queenmaker-plot` [event.conspiracy], `myrcella-is-maimed-by-darkstar` [event.incident]) + 3 bare Plate-3 Dorne nodes repaired (`arrest-of-the-sand-snakes`, `areo-hotah-springs-the-ambush`, `arianne-collapses-and-is-captured` — spaced aliases + quotes + clean bodies) + 11 edges (5 causal/agency Tier-2 fresh-subagent CONFIRMED + 6 role Tier-1). **ROOTED cross-book** at `gregor-confesses-and-kills-oberyn CAUSES arrest-of-the-sand-snakes` (walks 5 hops to Sansa's hairnet). **The Queenmaker plot is a declared standalone prime mover** (0-upstream by design — Arianne's inheritance-fear, an actor-internal motive; verifier endorsed NOT adding `arrest CAUSES plot`). **TRAP avoided:** `conquest-of-dorne`=Daeron I's historical conquest (AC 161), not the AFFC plot. `scripts/mint_dorne_myrcella_arc.py`. **19 new harvest rows pushed (Dorne POVs, several Quentyn/Essos-bridge seeds) — consume-pass now DUE.**

### NEXT BUILD: the ESSOS container (major-arc backlog #2)
The next big build. Needs its **own decomposition dip** (like WO5K — `working/wo5k-decomposition.md` is the template). It unblocks: the 4 parked Essos-bridge harvest rows (142–145); the S116 pre-placed bridge node `euron-commissions-victarion-to-fetch-daenerys` (auto-joins via the cross-book `--causal-chain` walk, S112); and the Quentyn/Dorne→Essos bridge (Doran's "Fire and blood" pact, harvest row this session). Decompose first (Drogo's death/dragon birth → Slaver's-Bay campaign → escape from Meereen + the Westeros↔Essos bridges: Robert's assassination order, Illyrio↔Varys conspiracy, Jorah-spies-for-Varys), then build junctures one at a time, dip-gated. **A harvest consume-pass is due first** (19 open rows + the "after any text-heavy session" rule).

## (historical) AFFC #2 Kingsmoot → Euron DONE (S116)
...
## (historical, DONE S117) AFFC #4 Dorne / Myrcella — the build brief that was executed

The last AFFC smoke-test fumble (#4). Built S117 (above). Original brief: **1–2 mints** + edges:
...
- **LOCAL root, NOT deepest ancestor (S117, Matt).** The root-check's "find the true antecedent" means the **local enabling cause** (Dorne's arrest thread roots at *Oberyn's death*), NOT "chain it as deep as the `--causal-chain` walk will reach." Causation is only weakly transitive: A causes B and B causes C does NOT make A a meaningful cause of C across many hops. As the graph densifies, one early super-event (`sansa-receives-the-poisoned-hairnet` — the Purple Wedding poisoning that decapitates House Lannister) becomes the transitive ancestor of more and more arcs (14 downstream as of S117: Joffrey's death, Tyrion's trial+escape, Tywin's death, Cersei's downfall, the Dorne arrest thread). That centrality is *earned* (GRRM designed it as the mid-game prime mover) — but do NOT **report** an arc by its deepest reachable ancestor ("walks N hops back to the hairnet" as a headline). Report the arc's LOCAL root + its standalone prime-mover thread; mention deep ancestry only as incidental connectivity, never as the achievement. Over-rooting (grafting every arc onto the deepest trunk to satisfy 5b) is the failure mode this guards against — if an arc's true local antecedent is itself standalone or shallow, that's correct.
