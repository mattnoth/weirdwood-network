...
## Parallel track (separate prompt)
The **D&E Pass-1-via-longrun.sh harness planning** track is parallel-safe with this one and has its OWN
continue prompt: `progress/continue-prompts/2026-06-29-de-longrun-harness-planning.md` (design session — wire
longrun.sh + a D&E orchestrator sub-script into the unified worker harness; launches nothing). Run it in a
separate window whenever; it doesn't block or depend on these remainders.
