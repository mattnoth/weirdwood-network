...
**Generated:** 2026-06-21 (refreshed after S121)  
...
|----------|------|-------|--------|-------------------|------|
| `2026-06-21-container-shape-analysis.md` | 2026-06-21 | dedicated CONTAINER-SHAPE analysis session (subagents active) | **LIVE** | Sonnet 4.6 | **New S121.** Step 1 HARDENING is DONE (shipped this session). This is the dedicated session Matt asked for to settle the container **SHAPE** — *shape > names* (tags are trivially reversible; the partition/boundaries/seams/granularity are the costly axis because arcs get built under them). Run the 4-lens fan-out (`…-container-split-BRIEFING.md`) → synthesize a shape map → **Matt picks the SET** (names provisional). Orchestrator first-draft to pressure-test: `…-container-split-PROPOSAL.md` (finds `riverlands`+`kl-faith` also container-sized). **Prereq: API must be healthy** — blocked S121 by a 529 outage. BUILD is a separate later session. |

---

## Archive (`archive/` subfolder — 37 files)
...
**LIVE: `2026-06-21-container-shape-analysis.md`** — Step 1 HARDENING shipped in full S121 (graph-query `--full-chain`/
`--expand-beats`/`--container`; `containers:` field adopted + 15 Essos nodes tagged; `littlefinger-betrays-ned` minted;
`sons-of-harpy→wedding` rebuilt as a 2-cause convergence hub via `galazza-counsels-the-ghiscari-marriage`; ENABLES/L1-L2
docs). The next session is a **dedicated CONTAINER-SHAPE analysis** (Matt's call — the container SET is a graph-shape
decision). **Governing principle: SHAPE > NAMES** — tags are trivially reversible (find-replace, no edges/traversal
touched), so don't agonize over labels; get the *shape* right (partition / boundaries / seams+ownership / Jon-Bran
granularity) because arcs get built under it. Run the 4-lens fan-out (`…-container-split-BRIEFING.md`) → synthesize →
**Matt picks the SET**. Orchestrator first-draft to pressure-test: `…-container-split-PROPOSAL.md`. **Prereq: API healthy**
(blocked S121 by a persistent 529 overload — every subagent dispatch died). BUILD (WO5K-remainder, seam-safe) is a
separate later session. (**Sonnet 4.6**)
