...
## NEXT BUILD (S114 endsession pick): AFFC #3 Brienne → Lady Stoneheart

Best cross-book auto-join — it roots at the **already-built `red-wedding`** node (Beric Dondarrion resurrects the murdered Catelyn → Lady Stoneheart → she hunts Freys/Lannisters and captures Brienne, forcing the sword-or-noose choice). Likely **2 mints** + ~3–4 edges: a Stoneheart-reveal beat (`lady-stoneheart-revealed` / `catelyn-resurrected-as-lady-stoneheart`) downstream of `red-wedding`, and a Brienne-captured-by-the-Brotherhood beat. **TRAP: `brienne-arrested` is the ASOS Harrenhal cell (Jaime/Balon Swann), NOT this — do not wire to it.** "Lady Stoneheart" resolves to the `catelyn-stark` character node, so dedup-check + mint the event beats. ASOS→AFFC (Brienne/Merrett-Frey POVs). Use SPACED aliases. Then re-check + run a harvest-consume pass (queue ~15, will cross the 20–30 threshold).

**Remaining AFFC queue:** #2 Kingsmoot→Euron (standalone — needs `death-of-balon-greyjoy` + a Euron-wins beat mint; retype `kingsmoot-on-old-wyk` event.battle→event.ceremony; near-bare) · #4 Dorne/Myrcella (needs `myrcella-wounded` mint; `arrest-of-the-sand-snakes` exists, bare). **Secondary cheap dip Q5** (`storming-of-the-crag CAUSES robb-weds-jeyne-westerling`, ASOS; dedup-check `battle-of-the-crag` first; theory-loaded Sybell-Spicer/moon-tea — do-not-lose). WO5K junctures (#3 Blackwater-upstream J2+J9 · #4 Karstark J7 · #5 Balon→Winterfell J4; SKIP J6): `working/wo5k-decomposition.md`.
