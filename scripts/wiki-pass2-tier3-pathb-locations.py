#!/usr/bin/env python3
"""Path B Sub-task 2 — Promote place.location + place.region nodes.

Driven by entity_type_guess in {'place.location', 'place.region'} from
...
Usage:
  python3 scripts/wiki-pass2-tier3-pathb-locations.py           # dry-run
  python3 scripts/wiki-pass2-tier3-pathb-locations.py --apply
  python3 scripts/wiki-pass2-tier3-pathb-locations.py --plan
"""
...
NODE_TYPE_DEFAULT = "place.location"
TARGET_TYPES = {"place.location", "place.region"}
DEST_DIR = GRAPH_LOCATIONS_DIR
...
def render_skeleton(
    ...
    if entity_type == "place.region":
        identity_line = f"{page_name} is a region from the AWOIAF wiki."
    else:
        identity_line = f"{page_name} is a location from the AWOIAF wiki."
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Path B Sub-task 2: promote place.location + place.region nodes."
    )
    ...
    if bad_slugs:
        print(f"  REGRESSION: {len(bad_slugs)} location nodes with slug ending in '.node':")
        for f in bad_slugs:
            print(f"    {f}")
    else:
        print(f"  OK: 0 location nodes with slug: *.node")

    print("\n=== Summary ===")
    print(f"Place pages in index:             {len(target_pages) + len(glossary_skipped)}")
    print(f"Glossary skips:                   {len(glossary_skipped)}")
    ...
    if args.apply:
        ...
        total_location_nodes = (
            len(list(DEST_DIR.glob("*.node.md"))) if DEST_DIR.is_dir() else 0
        )
        print(f"Total location nodes after pass:  {total_location_nodes}")
    else:
