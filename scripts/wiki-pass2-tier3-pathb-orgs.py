#!/usr/bin/env python3
"""Path B Sub-task 4 — Promote organization.house/religion/faction nodes.

Driven by entity_type_guess in {'organization.house', 'organization.religion', 'organization.faction'} from
...
Usage:
  python3 scripts/wiki-pass2-tier3-pathb-orgs.py           # dry-run
  python3 scripts/wiki-pass2-tier3-pathb-orgs.py --apply
  python3 scripts/wiki-pass2-tier3-pathb-orgs.py --plan
"""
...
GRAPH_HOUSES_DIR = PROJECT_ROOT / "graph" / "nodes" / "houses"
GRAPH_RELIGIONS_DIR = PROJECT_ROOT / "graph" / "nodes" / "religions"
GRAPH_FACTIONS_DIR = PROJECT_ROOT / "graph" / "nodes" / "factions"

TYPE_TO_DIR = {
    "organization.house": GRAPH_HOUSES_DIR,
    "organization.religion": GRAPH_RELIGIONS_DIR,
    "organization.faction": GRAPH_FACTIONS_DIR,
}
CONFLICTS_DIR = PROJECT_ROOT / "graph" / "nodes" / "_conflicts"
BUCKET_DIR = PROJECT_ROOT / "working" / "wiki" / "pass2-buckets" / "tier3-pathb-orgs"
...
NODE_TYPE_DEFAULT = "organization.house"
...
def render_skeleton(
    ...
    if entity_type == "organization.house":
        identity_line = f"{page_name} is a noble house from the AWOIAF wiki."
    elif entity_type == "organization.religion":
        identity_line = f"{page_name} is a religion/faith from the AWOIAF wiki."
    else:
        identity_line = f"{page_name} is an organization/faction from the AWOIAF wiki."
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Path B Sub-task 2: promote organization.* nodes."
    )
    ...
    if bad_slugs:
        print(f"  REGRESSION: {len(bad_slugs)} organization nodes with slug ending in '.node':")
        for f in bad_slugs:
            print(f"    {f}")
    else:
        print(f"  OK: 0 organization nodes with slug: *.node")

    print("\n=== Summary ===")
    print(f"Organization pages in index:             {len(target_pages) + len(glossary_skipped)}")
    print(f"Glossary skips:                   {len(glossary_skipped)}")
    ...
    if args.apply:
        ...
        print(f"Total org nodes after pass:      {total_org_nodes}")
    else:
