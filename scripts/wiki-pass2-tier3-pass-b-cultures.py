#!/usr/bin/env python3
"""Tier 3 Pass B — Promote missing culture nodes.

Builds and runs the Stage 3 pipeline for CULTURE_OF orphan slugs that have
corresponding wiki pages but no existing node in graph/nodes/factions/.

Type decision: organization.faction → factions/
  - architecture.md defines concept.culture but the promote script's TYPE_DIR_MAP
    maps concept.culture → concepts/, NOT cultures/. The task's decision rule says:
    if TYPE_DIR_MAP doesn't map to cultures/, fall back to organization.faction →
    factions/ (matching the 6 existing Tier 2 culture nodes: first-men, ironborn,
    free-folk, wildlings, dornishmen, dothraki).
  - New cultures join the factions/ directory. A future migration pass can move
    the entire cluster once a cultures/ directory is formally adopted.
...
Pipeline:
  ...
  9. Verify: slug regression check.

Usage:
  python3 scripts/wiki-pass2-tier3-pass-b-cultures.py           # dry-run
  python3 scripts/wiki-pass2-tier3-pass-b-cultures.py --apply   # write nodes
  python3 scripts/wiki-pass2-tier3-pass-b-cultures.py --apply --verbose
  python3 scripts/wiki-pass2-tier3-pass-b-cultures.py --plan    # print slug list only
"""
...
NODE_TYPE = "organization.faction"
DEST_DIR = GRAPH_FACTIONS_DIR
...
def load_existing_slugs_all() -> set[str]:
    """Return set of slugs already present anywhere under graph/nodes/."""
...
def load_culture_of_orphans() -> list[tuple[str, int]]:
    """Return [(slug, edge_count)] for CULTURE_OF orphans from cat1 TSV.
...
# ---------------------------------------------------------------------------
# Skeleton renderer (culture-specific — type is always organization.faction)
# ---------------------------------------------------------------------------
...
def render_skeleton(
    ...
    """Render a culture skeleton node as a markdown string.

    Type is always NODE_TYPE regardless of what the infobox/page-index says.
    """
    ...
    lines = [
        ...
        "## Identity",
        "",
        f"{page_name} is a culture/people from the AWOIAF wiki.",
        "",
        "## Edges",
        "",
    ]
...
# ---------------------------------------------------------------------------
# Prose extractor (matches wiki-pass2-tier3-pass-a-titles.py conventions)
# ---------------------------------------------------------------------------
...
def build_manifest(pages: list[tuple[str, str]]) -> dict:
    """Build manifest dict for the tier3-cultures bucket."""
    return {
        "bucket_id": BUCKET_ID,
        "tier": "secondary",
        "tier_default": "tier-1",
...
def check_slug_regression() -> list[str]:
    """Return list of culture node files where slug ends in '.node' (regression bug)."""
    bad: list[str] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Tier 3 Pass B: promote missing culture nodes from CULTURE_OF orphans."
    )
    parser.add_argument(
    ...
    # ------------------------------------------------------------------
    orphan_slugs = load_culture_of_orphans()
    print(f"CULTURE_OF orphan slugs from TSV: {len(orphan_slugs)}")
    ...
    if args.apply:
        ...
        print(f"  {BUCKET_DIR / 'manifest.json'}")
    ...
    for slug, page_name, edge_count in promotable:
        ...
        if infobox_found:
            ...
            if inferred_type and inferred_type != NODE_TYPE and inferred_type != "unknown":
                ...
                if args.verbose:
                    print(f"  TYPE OVERRIDE: {page_name}: {inferred_type} -> {NODE_TYPE}")
        ...
        if re.search(r"^slug:.*\.node\s*$", skeleton_content, re.MULTILINE):
            print(f"  ERROR: slug regression detected for {page_name}", file=sys.stderr)
    ...
    # ------------------------------------------------------------------
    # Step 8: Promote (concatenate skeleton + prose → graph/nodes/factions/)
    # ------------------------------------------------------------------
    print(f"\n--- Promoting nodes to {DEST_DIR} ---")

    if not args.apply:
        print(f"  [DRY-RUN] Would promote {skeleton_count} nodes to: {DEST_DIR}")
    else:
    ...
    if bad_slugs:
        print(f"  REGRESSION: {len(bad_slugs)} culture nodes have slug ending in '.node':")
        for f in bad_slugs:
            print(f"    {f}")
    else:
        print(f"  OK: 0 culture nodes with slug: *.node")

    # ------------------------------------------------------------------
    # Summary
    # ------------------------------------------------------------------
    print("\n=== Summary ===")
    print(f"CULTURE_OF orphan slugs in TSV:   {len(orphan_slugs)}")
    ...
    if args.apply:
        ...
        total_faction_nodes = len(list(DEST_DIR.glob("*.node.md")))
        print(f"Total faction nodes after pass:   {total_faction_nodes}")
    else:
