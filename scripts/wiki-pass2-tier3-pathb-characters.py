#!/usr/bin/env python3
"""Path B Sub-task 5 — Promote character.* nodes.

Driven by entity_type_guess in {'character.human', 'character.dragon', 'character.direwolf'} from
...
Usage:
  python3 scripts/wiki-pass2-tier3-pathb-characters.py           # dry-run
  python3 scripts/wiki-pass2-tier3-pathb-characters.py --apply
  python3 scripts/wiki-pass2-tier3-pathb-characters.py --plan
"""
...
GRAPH_CHARACTERS_DIR = PROJECT_ROOT / "graph" / "nodes" / "characters"
CONFLICTS_DIR = PROJECT_ROOT / "graph" / "nodes" / "_conflicts"
BUCKET_DIR = PROJECT_ROOT / "working" / "wiki" / "pass2-buckets" / "tier3-pathb-characters"
...
NODE_TYPE_DEFAULT = "character.human"
TARGET_TYPES = {"character.human", "character.dragon", "character.direwolf"}
DEST_DIR = GRAPH_CHARACTERS_DIR
...
def render_skeleton(
    ...
    if entity_type == "character.dragon":
        identity_line = f"{page_name} is a dragon from the AWOIAF wiki."
    elif entity_type == "character.direwolf":
        identity_line = f"{page_name} is a direwolf from the AWOIAF wiki."
    else:
        identity_line = f"{page_name} is a character from the AWOIAF wiki."
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Path B Sub-task 2: promote character.human + character.dragon nodes."
    )
    ...
    if bad_slugs:
        print(f"  REGRESSION: {len(bad_slugs)} character nodes with slug ending in '.node':")
        for f in bad_slugs:
            print(f"    {f}")
    else:
        print(f"  OK: 0 character nodes with slug: *.node")

    print("\n=== Summary ===")
    print(f"Character pages in index:             {len(target_pages) + len(glossary_skipped)}")
    print(f"Glossary skips:                   {len(glossary_skipped)}")
