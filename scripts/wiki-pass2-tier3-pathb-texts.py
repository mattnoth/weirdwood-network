#!/usr/bin/env python3
"""Path B Sub-task 1a — Promote in-world text/book/song nodes.

Driven by entity_type_guess == 'object.text' from page-index.jsonl
(post-categorizer Path B classification). Mirrors the Tier 3 Pass B
(cultures) pipeline structure: build manifest -> emit skeletons ->
extract prose -> concatenate -> atomic-rename to graph/nodes/texts/.

Real-world publications are excluded upstream by the parser's
"Books-only without Books and scrolls" SKIP rule (see
wiki-infobox-parser.py::classify_by_categories).
...
Pipeline:
  ...
  6. Slug-regression check.

Usage:
  python3 scripts/wiki-pass2-tier3-pathb-texts.py           # dry-run
  python3 scripts/wiki-pass2-tier3-pathb-texts.py --apply   # write nodes
  python3 scripts/wiki-pass2-tier3-pathb-texts.py --plan    # print slug list only
  python3 scripts/wiki-pass2-tier3-pathb-texts.py --apply --verbose
"""
...
TARGET_TYPE = "object.text"
DEST_DIR = GRAPH_TEXTS_DIR
...
# Data loaders
# ---------------------------------------------------------------------------
def load_target_pages() -> list[tuple[str, str]]:
    """Return [(slug, page_name)] for all pages with entity_type_guess == TARGET_TYPE."""
...
def render_skeleton(
    ...
    lines = [
        ...
        "## Identity",
        "",
        f"{page_name} is a text/book/song from the AWOIAF wiki.",
        "",
        "## Edges",
        "",
    ]
...
# ---------------------------------------------------------------------------
# Prose extractor (matches tier3-pass-b conventions)
# ---------------------------------------------------------------------------
...
def extract_prose(page_name: str) -> str | None:
    # Wiki cache encodes colons as underscores (": " in title -> "__" on disk).
    safe_filename = (
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Path B Sub-task 1a: promote object.text nodes."
    )
    ...
    print(f"Pages with entity_type_guess == {TARGET_TYPE!r}: {len(target_pages)}")
    ...
    if bad_slugs:
        print(f"  REGRESSION: {len(bad_slugs)} text nodes with slug ending in '.node':")
        for f in bad_slugs:
            print(f"    {f}")
    else:
        print(f"  OK: 0 text nodes with slug: *.node")

    # Summary
    print("\n=== Summary ===")
    print(f"object.text pages in index:       {len(target_pages)}")
    ...
    if args.apply:
        ...
        print(f"Total text nodes after pass:      {total_text_nodes}")
    else:
