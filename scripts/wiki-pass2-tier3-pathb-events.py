#!/usr/bin/env python3
"""Path B Sub-task 3 — Promote event.battle + event.war nodes.

Driven by entity_type_guess in {'event.battle', 'event.war'} from
...
Usage:
  python3 scripts/wiki-pass2-tier3-pathb-events.py           # dry-run
  python3 scripts/wiki-pass2-tier3-pathb-events.py --apply
  python3 scripts/wiki-pass2-tier3-pathb-events.py --plan
"""
...
GRAPH_EVENTS_DIR = PROJECT_ROOT / "graph" / "nodes" / "events"
CONFLICTS_DIR = PROJECT_ROOT / "graph" / "nodes" / "_conflicts"
BUCKET_DIR = PROJECT_ROOT / "working" / "wiki" / "pass2-buckets" / "tier3-pathb-events"
...
# These match the regex but aren't actually events:
#   - Ships ("Lance (war galley)")
#   - Books about wars ("Conquest of Dorne (book)", "Account of the War...")
#   - Timeline meta-pages ("Years after Aegon's Conquest" + Calculations subpages)
#   - Factions / character groups ("Blacks", "Greens", "Engines of War",
#     "Lads", "Brothel Queens", "Two Alans", "Two Betrayers", "Seven Who Rode",
#     "Winter Wolves", "Widow Fairs", "Gutter knights", "Sowing")
#   - Ceremonies ("Coronation of...", "Wedding of...")
# Tightening the parser's PAGE_NAME_TYPE_PATTERNS to exclude these would
# require list-of-exclusions logic anyway; cleaner to skip them here.
GLOSSARY_SKIP_PAGES: set[str] = {
    "Lance (war galley)",
    "Conquest of Dorne (book)",
    ...
    "Years after Aegon's Conquest/Calculations Ages (Continued2)",
    ...
    "Coronation of Aegon II Targaryen",
    "Coronation of Rhaenyra Targaryen",
    "Wedding of Aegon III Targaryen and Jaehaera Targaryen",
}
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Path B Sub-task 2: promote event.battle + event.war nodes."
    )
    ...
    if bad_slugs:
        print(f"  REGRESSION: {len(bad_slugs)} event nodes with slug ending in '.node':")
        for f in bad_slugs:
            print(f"    {f}")
    else:
        print(f"  OK: 0 event nodes with slug: *.node")

    print("\n=== Summary ===")
    print(f"Event pages in index:                 {len(target_pages) + len(glossary_skipped)}")
    print(f"Glossary skips:                   {len(glossary_skipped)}")
    ...
    if args.apply:
        ...
        print(f"Total event nodes after pass:     {total_event_nodes}")
    else:
