...
| 9 | Verify / drift gate | reuse fresh-verify subagents + schema validator (extended: dispute-axis strata, §7) | **EXERCISED ✅ S199** — §7.2 stratified dispute-axis audit ran twice on p02: v1 **FAIL** (26.9% missed passage-scope hedges) → reconciler **dispute-proximity quarantine** built (`dispute-review.jsonl`, strong-terms lexicon ±1 paragraph-line, certainty rescue, romance-class hold) → v2 **PASS** (0.0%/0.0%). Out-of-sample accept validation PASS (0 wrong/66). Per-unit fresh-verify remains mandatory at apply (adjudicates held rows; verdict-gates-apply) |
