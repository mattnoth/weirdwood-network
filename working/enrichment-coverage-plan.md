...
|:--:|---------|---------------------------|----------------------|-------|
| ~~**D1**~~ | ~~**Battle of Castle Black / the Wall defense** (ASOS)~~ **✅ DONE S153** | Mance's assault on the Wall, Jon's defense, the giants & mammoths, the burning of the gate, Ygritte's death, Donal Noye, the relief by Stannis. A MAJOR current-narrative battle that no single POV "arc" owns (Jon/Wall S145 enriched the *leadership* arc, not this battle). | ~~likely thin/islanded battle nodes~~ → was: dense social/dyad layer but **thin event-hub structure**; hub had NO causal upstream | **DONE S153** (Matt-picked at the fork): +6 nodes / +34 net edges; reified the marquee under-built beats (death-of-ygritte, Noye&Mag tunnel-death, the southern Thenn assault, Jon-takes-command, Jon's Mance-sortie, Bowen-marches-the-garrison); lit the upstream (fight-at-the-fist + bowen-march ENABLE the hub → `--full-chain` walks great-ranging→…→Jon's assassination, 16 chain edges); whodunit layer (Varamyr eagle-recon, Jon/Mance mutual deception, Slynt coercion). Held the line: NO `jon KILLS styr` (the Wall kills him), NO Ygritte-attacker (un-attributed), NO Grenn-gate (show-only). |
