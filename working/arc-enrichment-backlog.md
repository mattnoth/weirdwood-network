...
|------|------|:-----------:|-----------|-------|
| Kingsmoot → Euron | arc/cluster | **2** | S157 | **S157 (pass 2, A-roundup #3): +37 edges / 0 new nodes** — heaviest-dedup dip (spine+enrich+voyage all pre-built; baseline 230 internal/11 causal). Lit the 0-role taking-of-the-shields hub (victarion/euron roles + 4 shield-lords APPOINTS + victarion-slays SUB_BEAT_OF), the full kingsmoot (naggas-hill LOCATED_AT, aeron OFFICIATES, dragonbinder WIELDED_IN + cragorn, 3 claimants + red-rain, Asha's champions), the Reader (rodrik-harlaw UNCLE_OF/ADVISES/ALLIES_WITH), 0-edge objects (silence/iron-victory CAPTAIN_OF, euron OWNS dragonbinder), whodunit (euron MANIPULATES hotho via_bribe — NO new SUSPECTED_OF), salt-wife KILLS + burning-of-the-lannister-fleet seam. DROPPED the S116 `anarchy-in-the-reach` TRAP. Fresh-verify 36C/1A/0R, 0 drift. **S116 (pass 1):** +2 beat-nodes, +6 edges, +1 type (`SUSPECTED_OF`), 3 bridge nodes. |
...
| Arya's flight & Harrenhal (D5) | arc/cluster (Class-D event cluster) | **1** | S154 | Eighteenth major-arc dip — the SECOND **Class-D** event cluster (Matt-picked at the fork over D2/D3/A2; natural S150/S153 follow-on). Big AGOT→ACOK Arya journey; dense social/dyad layer but a causally-ISLANDED event spine. +6 nodes (`stableboy-killing`, `death-of-weese` [event.death]; `arya-names-jaqen-himself`, `jaqen-gives-arya-the-iron-coin`, `arya-escapes-harrenhal`, `gold-cloaks-demand-gendry` [event.incident] — **NO container tag**) +44 −1 (E1) = **net +43** (edges 23,022→**23,065**; events/ 720→726). **MARQUEE iron-coin ORIGIN seam:** `jaqen-gives-arya-the-iron-coin ENABLES arya-departs-for-braavos` threads the cluster into the S150 Braavos arc — `--full-chain` from the terminal Braavos assassination now walks **14 edges** back to `fight-at-the-holdfast`. **Fixed both dead-ends:** holdfast (0 causal → CAUSES arya-captured + role edges) + `needle` (0 out → 2 WIELDED_IN: stableboy-kill + holdfast). **Reified:** Needle's first kill (the stableboy, agot-arya-04), Jaqen's 2nd death (Weese), **the naming-gambit** (Arya names Jaqen himself → CAUSES the weasel-soup `guards-killed`, which had 0 causal-in), the escape, the Cersei→Gendry gold-cloak demand. **Cross-arc seams:** `fall-of-harrenhal ENABLES red-wedding-conspiracy`; `daily-interrogations-and-killings SEEKS beric-dondarrion`; `incident-at-the-trident MOTIVATES arya` (fixed 0-out); `brave-companions CAUSES fall`; `roose-bolton RULES harrenhal`. Re-cited `iron-coin GIFTED_TO arya` (ASOS recollection → acok-arya-09:327 actual giving). **Fresh-verify 8C/1A/0R, theory-gate CLEAN, 3 traps held** (no Needle-in-escape [Bolton's dagger]; no MANIPULATES-on-gambit [Jaqen aware = open coercion]; no tywin-COMMANDS-holdfast [wrong quote]); **DROPPED E1** `gregor-raids ENABLES holdfast` (campaign scope mismatch — AGOT opening-raids node ≠ ACOK chevauchée). 0 drift (43/43), 0 off-vocab (15 types, no first-use). Mint `scripts/mint_d5_arya_harrenhal_enrichment_s154.py` + `finalize_d5_arya_harrenhal_s154.py`; artifacts `working/enrichment/d5-arya-harrenhal/`. **37 harvest rows pushed.** **Pass-2:** the Tickler torture as node-prose/event; the Harrenhal node-tangle alias-hygiene (§Small Fixes); descriptive depth on harrenhal/iron-coin/vargo/weese/roose nodes. |
| Stannis Baratheon (A2.7) | arc/cluster (A-roundup) | **1** | S155 | Nineteenth major-arc dip — the **FIRST of the 🅰 A-roundup campaign** (Matt S154). Stannis is NOT a POV; his arc spreads across Davos/Catelyn/Jon/Asha POVs + containers, and is heavily pre-wired via Blackwater (S138) + NORTH (S125-126) — so WIRE+ENRICH, not a build. Baseline = **409 internal edges but only 10 internal CAUSAL edges** (the Class-D islanded-hub profile); the dedup killed most surface proposals. +3 nodes (`burning-of-the-seven-at-dragonstone`, `leeching-of-edric-storm` [event.incident]; `shadow-killing-of-cortnay-penrose` [event.assassination] — **NO container tag**, Dragonstone/Renly's-war) +31 edges, 0 dropped → **net +31** (edges 23,065→**23,096**; events/ 726→729). **MARQUEE — the R'hllor conversion engine** (spine had nothing): minted the burning-of-the-Seven, **lit `lightbringer`'s 0-edges-graph-wide island** (stannis WIELDS + WIELDED_IN), added the two single-most-missing edges `stannis WORSHIPS rhllor` + `melisandre ADVISES stannis`. **The SECOND shadow** (kept DISTINCT from shadow-assassination-of-renly) ENABLES `taking-of-storms-end`; the Renly-death witness/whodunit (catelyn+brienne WITNESS_IN; brienne+catelyn SUSPECTED_OF = the false in-world accusations). **The MOTIVATES substrate:** `renly-s-death-reflection MOTIVATES stannis` (kinslaying-guilt, de-islanded), `davos MOTIVATES stannis` (why-march-north), cross-container seam `murder-of-jon-arryn MOTIVATES stannis` (RR/twincest claim-root). Leeching of Edric Storm as TEXT event → MOTIVATES davos → `davos RESCUES edric`. **Fresh-verify 27C/4A/0R, theory-gate CLEAN** (Azor Ahai / king's-blood-magic / stone dragon / Shireen-sacrifice held node-prose); finalize E21→T2 + E22 note + 10 stamps; **rejected WORSHIPS→FOLLOWS** (FOLLOWS not in vocab). 0 drift (31/31), 0 invented types. Excluded wiki name-collisions (flight-to/fall-of-dragonstone, shadow-war = Dance/Slaver's-Bay). Mint `scripts/mint_stannis_enrichment_s155.py` + `finalize_stannis_s155.py`; artifacts `working/enrichment/stannis/`. **40 harvest rows pushed.** **Pass-2:** the Antler-Men conspiracy (B5, its own dip); `melisandre PRACTICES shadowbinding` + a `shadowbinding` concept node; the onion-knight RR backstory (Storm's End siege); Dragonstone/Lightbringer descriptive depth; the queen's-men sacrifice-trio node-tangle. |
| Dorne / Queenmaker (A1.5) | arc/cluster (A-roundup) | **1** | S156 | Twentieth major-arc dip — the **SECOND of the 🅰 A-roundup campaign** (Matt S154); the whodunit/informer wheelhouse. The Dorne spine was built S117 (the Queenmaker plot) with **0 enrichment dips**; baseline = **196 internal edges but only 9 internal CAUSAL** (thin causal spine + under-wired conspirator web) — WIRE+ENRICH; the dedup killed the dense dyadic web. +3 nodes (`garin-of-the-orphans` [character.human — fixes a wrong-target bug], `hotahs-longaxe` [object.artifact], `spear-tower` [place.location] — **NO container tag**, Dorne not approved) +39 minted −1 fresh-verify-reject −2 bug-drops = **net +36** (edges 23,096→**23,132**; nodes 8,686→**8,689**; events/ steady 729). **MARQUEE — the Arys 'soiled knight' sub-arc** (4-lens converge): `arianne MANIPULATES arys-oakheart` via_seduction (the half-year seduction weaponizing the Kingsguard; LOVER_OF pre-existed) + `DECEIVES arys` by_lie (the false marriage promise) + `arys BREAKS_VOW tommen` + `arys ATTACKS areo-hotah` (the suicide charge). Conspirator web: Garin/Drey/Sylva AGENT_IN+CONSPIRES_WITH + dispersal (Drey→Norvos, Garin→Tyrosh, Sand Snakes IMPRISONED_AT the new spear-tower). **THE WHODUNIT DONE HONESTLY:** **NO informer `SUSPECTED_OF` minted** — the text leaves "someone always tells" deliberately unproven (Arianne cycles through suspects, reaches no conclusion; Doran refuses to name them); fresh-verify CONFIRMED the non-mint as correct. The whodunit value instead = `doran MANIPULATES arianne` via_false_information (the structural irony — the false-suitor ploy concealing the pact CAUSED the plot) + cover-story `DECEIVES balon-swann` ×2 (Darkstar-killed-Arys) + `gerold-dayne OPPOSES the-queenmaker-plot` (the saboteur-within — AGENT_IN the act yet scorned the method, maimed Myrcella to spark the war the plot avoided). **CROSS-ARC SEAMS:** `murder-of-elia... MOTIVATES doran-martell` (Sack-of-KL→Dorne; the graph had only `...MOTIVATES oberyn`) + `myrcella CLAIMS iron-throne` (**FIRST CLAIMS-iron-throne edge in the graph**) + the Sand-Snake forward-dispatch (Obara→High-Hermitage, Nym/Tyene→KL). De-islanded `water-gardens` (was 0 edges to core), `hotahs-longaxe` (0→3), `ghaston-grey`/`sea-of-dorne`/`spear-tower`. **BUG-FIX (finalize):** 4 pre-existing edges mis-pointed the conspirator Garin at the **legendary** `garin-the-great` (Rhoynar prince) — retired 2 (replaced by the new node) + re-pointed 2 (the Cedra↔Garin fling); the 4 genuine legendary edges untouched. **Fresh-verify 35C/4A/0R, theory-gate CLEAN** (Aegon-is-real/fAegon/fire-and-blood prophecy/R+L-Elia held node-prose): DROPPED **E15** `doran MARRIES_OFF sylva` (her FATHER shipped her to Greenstone, not Doran); note-sharpened E1+E19. 0 drift (38/38), 0 invented types (`TRAVELS_TO` 1→7). Mint `scripts/mint_dorne_enrichment_s156.py` + `finalize_dorne_s156.py`; artifacts `working/enrichment/dorne/`. **27 harvest rows pushed.** **Pass-2:** the Tickler/torture layer; Tyene's poison craft (Oberyn-taught); the Old Palace / Tower of the Sun descriptive depth; a `garins-curse`/Greenblood-orphans sub-cluster; the Quentyn-to-Dany pact seam (when Essos/A2.4 lands). |
...
4. **Consume the harvest pointers** the dip refilled (attach load-bearing quotes to existing
   ...
   **if ≥ ~30 open, DRAIN it before close-out OR stage a dedicated drain as the next continue
   prompt — automatically, every dip, without waiting for Matt to notice the balloon.** This is
   the close-out trigger that lapsed S153–S156 (queue silently grew 0→120); it is now mandatory
   in the endsession checklist (step 0) too. A dip adds ~25–40 rows, so the ceiling trips ~every dip.
