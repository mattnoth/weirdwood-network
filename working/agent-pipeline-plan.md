...
| 18 | `orphan-edge-finder` | `.claude/agents/orphan-edge-finder.md` | **WRITE NOW (priority)** | After alias-resolver runs, finds edges whose targets resolve to no node (after considering aliases). Surfaces real graph gaps (vs. mere slug mismatches). Output: `working/audits/orphan-edges-<date>.md`. Read-only. |
