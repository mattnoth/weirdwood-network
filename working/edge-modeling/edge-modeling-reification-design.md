...
- **Role edge** — an edge whose type names the participant's thematic role in an event: `AGENT_IN` (executor), `VICTIM_IN` (patient), `COMMANDS_IN` (instigator/orderer), `WIELDED_IN` (instrument), `LOCATED_AT` (place). Source = participant, target = `event.*` node. **CORRECTION 2026-06-14:** For `LOCATED_AT` specifically, the canonical live-data direction is `event → location` (source=event, target=location) — NOT `location → event`. This supersedes the "Source = participant, target = event.*" phrasing above for LOCATED_AT only. All other role-edge types (AGENT_IN, VICTIM_IN, COMMANDS_IN, WIELDED_IN) retain `source = participant, target = event.*`. Do not flip any live data — the live `edges.jsonl` is correct.
