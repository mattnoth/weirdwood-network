#!/usr/bin/env python3
"""Whole-file line-check for Sansa/Vale enrichment candidates: for each edge, search the
...
CAND = ROOT / "working/enrichment/sansa-vale/candidates.json"
...
def main():
    ...
    for e in edges:
        ...
        if hits:
            ...
            print(f"PASS {e['id']:5} {e['source']:30.30} -{e['type']:14}-> {e['target']:34.34} L{hits[0]}{flag}")
            npass += 1
        else:
