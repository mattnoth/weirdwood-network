# Shared lens instructions — A2.7 Stannis enrichment (S155)

You are one of four parallel "lens" subagents enriching the **Weirwood Network**, a structured knowledge graph
for A Song of Ice and Fire. We are doing an **enrichment dip** on the **A2.7 Stannis Baratheon arc** — deepening
an already-heavily-built skeleton with the side-plots, revelation-events, descriptive/object depth, and (above
all) the **causal/MOTIVATES substrate** the first pass missed. Stannis is **NOT a POV**; his arc is told across
Davos/Catelyn/Jon/Asha POVs and spreads across containers. He is **heavily pre-wired** (the dedup pull found 409
internal edges but only 10 internal causal edges) — so this is **WIRE + ENRICH**, and **the dedup will kill many
proposals**. That is expected. Aim your proposals at the GAPS, not the dense dyadic web.
...
1. `working/enrichment/stannis/baseline.md` — the scope, the 409-edge existing web, the 10-edge causal spine,
   the DEAD-ENDS/GAPS (your targets), and the DO-NOT list. **Every node/edge you propose MUST be deduped against
   the existing web in baseline.md.** If an edge already exists, do NOT re-propose it.
2. Your assigned chapter files (listed in your specific prompt). Read them in full. **Quotes must be VERBATIM and
   carry a `chapter:line` cite** (e.g. `acok-davos-02.md:21` → cite as `acok-davos-02:21`). A quote must be a
...
- **Causal ladder (honor it — load-bearing):** `TRIGGERS` = immediate spark, B is the very next beat.
  ...
  choice/third party actually produces B (the relief valve for the "sequence trap" — a campaign's place→place
  transitions are ENABLES, never CAUSES). `MOTIVATES` = drives an *actor's* decision; **target is a CHARACTER,
  never an event.** Use MOTIVATES to route Stannis's choices (the guilt over Renly; the "save the realm" decision)
  instead of collapsing them into a false event-CAUSES-event.
...
- `SUSPECTED_OF` (Tier-2, NEVER Tier-1) = a character is suspected (in-world or by reader) of being the agent/cause
  of an event the published text does NOT prove. (Catelyn names Stannis Renly's killer "through sorcery" — but note
  the shadow's agency is ALREADY wired stannis AGENT_IN; use SUSPECTED_OF only for genuinely UNproven agency.)
- `WORSHIPS` (person→religion), `PRACTICES` (person→concept.magic discipline), `SACRIFICES` (sacrificer→victim,
  ritual killing), `ADVISES` (advisor→advised), `RESCUES` (rescuer→rescued) — all take NO qualifier.
- `WIELDS` target MUST be `object.artifact` (e.g. `stannis WIELDS lightbringer`). `WIELDED_IN` = artifact→event.
...
(Dragonstone's leeches, the burning sacrifices, Stannis eating rats during the Storm's End siege, Crofter's-Village
starvation/horsemeat, the peach at the parley, prison rations). Also capture: load-bearing verbatim quotes (for node
`## Quotes`), vivid physical descriptions (Stannis the gaunt grinding-jawed king, Dragonstone's carved dragons & the
Chamber of the Painted Table, the nightfires, Lightbringer the red sword, Mel's ruby, Patchface, the smoking
Dragonmont), hospitality moments, and foreshadowing. POINT, don't extract — a later harvest pass attaches them.

## OUTPUT — write your proposal to `working/enrichment/stannis/lens-<X>.md`
(where `<X>` is your lens letter, given in your prompt). Structure:

```
# Lens <X> — <name> — A2.7 Stannis proposal (S155)

## Proposed NEW nodes
For each: slug (kebab-case) · name · type (from the entity hierarchy, e.g. event.incident / event.death /
event.battle / object.artifact / concept.magic) · 1-2 sentence body summary · the verbatim anchor quote + chapter:line.
...
- **Dedup** against baseline.md's 409 edges + the 10-edge causal spine — do not re-propose existing edges or the
  dense dyadic web. The dedup is EXPECTED to kill many proposals; aim at the GAPS.
- **Verbatim quotes only**, single contiguous substring of one line, with `chapter:line`. If you can't find the line, don't propose the edge.
- **No theory assertions** (GATED): Azor Ahai / Stannis-as-the-prince-that-was-promised / R'hllor cosmology /
  king's-blood-magic-WORKS / Shireen-as-future-sacrifice / the stone dragon. The leeching, the burning of the Seven,
  Mel's visions, "valar"-class words are TEXT EVENTS (propose the events + agency/possession edges); the theology
  stays node-prose, evidence-only. Do NOT propose `leeching FORESHADOWS/CAUSES` the three kings' deaths.
- **No show-canon** — only beats in the BOOK text (quote it from your chapter files or don't propose it).
- **Do NOT re-touch** the Blackwater wildfire (S138) or the NORTH Mance-glamour/"Rattleshirt" thread (S145) —
  confirm-only any seam into them.
- **No container tags** in your proposal (the synthesis decides; default none).
