#!/usr/bin/env python3
"""Whole-file line-check for AEGON enrichment candidates: for each edge, search the
...
CAND = ROOT / "working/enrichment/aegon/candidates.json"
...
def main():
    ...
    for e in edges:
        ...
        if hits:
            ...
            print(f"PASS {e['id']:5} {e['source']:24.24} -{e['type']:14}-> {e['target']:36.36} L{hits[0]}{flag}")
            npass += 1
        else:
