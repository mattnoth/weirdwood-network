...
| Q11 | Describe some detailed meals in the books. | thematic | *(no single entity phrase — this is the S188 live-failure case)* | `lemon-cake`, `honeyed-ham`, `bowl-of-brown`, `fish-stew`, `guest-right` (feast context) | search (shipped, step 5) / list (shipped, wiring pass 2) / theme (shipped, wiring pass 2) | **The S188 live failure**: 13 fuzzy/no-match resolves, loop-bound, no answer. This is the row the meals-question improvement metric (design doc §2b/D-I) must show flipping. Wiring pass 2 (2026-07-04) wired `list_nodes`/`theme` as chat tools — `theme("meals & feasts")`'s first page (25-row cap) already contains 2 of the 5 target slugs (`bowl-of-brown`, `guest-right`); the routing table now sends aggregative "describe some X" questions to list/theme FIRST. No longer loop-bound by construction — see post-step5b re-run. |
...
| Q16 | What does the graph say about hospitality customs (guest right, welcoming guests)? | thematic (hospitality) | `guest right`, `hospitality` | `guest-right`, `catelyn-secures-guest-right` | resolve → read; theme (shipped, wiring pass 2) hospitality seed theme | Required hospitality thematic question. `guest-right` resolves exactly today (2 quotes) and `catelyn-secures-guest-right` is a related event node — a reasonable traversal answer exists now via resolve+read, unlike Q11; this row is the "thematic archetype that already mostly works" control, useful for contrast against Q11's total failure. `theme("hospitality")` also reaches `guest-right` directly (theme has only 2 members total, `fosterage` + `guest-right` — no cap truncation issue at all); `catelyn-secures-guest-right` is an event node, not covered by any theme, so theme reaches 1 of 2 targets, same as resolve. |
