# Orphan Edges Audit — 2026-05-01

**Nodes scanned:** 4951
**Total edges checked:** 19054
**Edges that resolve cleanly:** 15789
**Orphan / problematic edges found:** 3265

**Breakdown:**
- Category 1 (target genuinely missing): 2955 edges across 1174 unique missing targets
- Category 2 (alias-mismatch — resolves via alias-resolver): 268 edges across 34 unique alias slugs
...
Sorted by `in_count` desc, then by edge_count. Top 100 of 1174 unique missing targets shown.
...
_(Long tail: 1074 additional missing targets. See `working/audits/orphan-edges-2026-05-01-cat1-full.tsv` for the complete list.)_

---

## Category 2: alias-mismatch (resolvable via alias-resolver)

These targets fail direct slug-match but DO resolve via the alias-resolver. **268 edges across 34 unique alias slugs** are affected.
...
| `andal` | `andals` | 54 | `alester-ii-arryn` / CULTURE_OF / Andal |
...
Of 19054 edges across 4951 nodes, 15789 resolve cleanly. 3265 are orphan or stale, dominated by Cat 1 (2955 edges, 1174 unique slugs). Date-bleed status: 42 stragglers remain. Religion-bleed leftovers: 0. Title-like targets are a structural gap: 21 title slugs referenced by 68 edges with no node. Cat 2 alias-mismatches (268 edges) are not gaps — they're handled the moment the graph layer consults `working/wiki-parsed/alias-resolver.json` after a direct-slug miss.
...
4. **MED — Wire alias-resolver into the graph query layer.** All 268 Cat 2 mismatches resolve once lookup consults `alias-resolver.json` on direct-slug miss. Avoids touching 34 source nodes.
