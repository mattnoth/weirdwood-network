...
**Status:** **TRACK COMPLETE — CLOSED DE-4 (2026-07-19), file frozen to `history/`.** Extraction 24/24 part-units verified (DE-3); the 4 TMK epithet-SAME_AS rows adjudicated **KEEP as alias capture** (DE-4); downstream work (graph-build integration · harvest sidecar drain · v4 back-port) lives in `working/todos.md` item (c2).
...
### Session DE-4 — CLOSE-OUT: epithets adjudicated KEEP, track frozen (2026-07-19)
**Model:** Fable 5 (orchestrator; same session then proceeds to the gated graph-build integration as a graph-track session on Matt's directive "orchestrate… anything else to get dunk n egg deployed"). **Type:** ADJUDICATION + BOOKKEEPING (no extraction, no graph mutation in this close-out).
**Adjudication (closeout step 1):** the 4 TMK epithet-SAME_AS rows (`tmk-dunk-01-p05` :287 Dunk↔the Gallows Knight / :290 Glendon Ball↔the Knight of the Pussywillows; `tmk-dunk-01-p06` :269 Dunk↔the Gallows Knight / :271 Glendon Ball↔the Knight of the Pussywillows) → **(a) KEEP as-is, as alias capture.** Rationale: mystery-knight epithets are exactly the resolver surface a chat user hits ("who was the Gallows Knight?"); the graph-build ingest RULE is that SAME_AS rows whose target is an epithet phrase (no distinct node) become **aliases on the bearer's node — never cross-identity edges** — which fully answers the auditor's epithet≠identity concern, losslessly and reversibly. Matt may veto at integration review (rows remain in the extraction files untouched).
**Freeze (step 2):** this file `git mv` → `history/worklog-dunk-egg.md`; `worklog.md` STATUS D&E line → COMPLETE/frozen; CLAUDE.md track-aware branches pointed at the frozen file.
**Steps 3–4:** downstream pointers already live in `working/todos.md` (c2) — verified adequate; `reference/pov-characters.md` carries no stale no-Pass-1 claim — no edit needed.
**What's next:** D&E graph-build integration (gated graph track, S-numbered) → harvest sidecar drain (372 rows) → deploy. Fired immediately after this close-out in the same orchestration session.
