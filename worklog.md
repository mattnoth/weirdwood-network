...
### STATUS — at a glance (verified 2026-06-19, S108)

**SHIPPED**
- Pass 1 mechanical extraction: **344/344 chapters, all 5 books** (done 2026-05-06, all Opus)
- Wiki cache local (17,945 fetched → 17,657 unique files) + Pass 2 promotion: **graph/nodes/ = 8,542** (events incl. S104 RR spark-beats + 5 S105 Bran's-fall beats + 8 S106 arc beats [4 Sack-of-KL + 4 Purple-Wedding] + 6 S107 beats [5 B1 Red-Wedding-upstream + 1 B2 Theon-ward] + **3 S108 beats [B3 Ned's-downfall: death-of-robert-baratheon + parentage-discovery + treason-confession]**; excl. `_conflicts/` staging)
- Entity + chapter indexes: **all 21 categories** (S72)
- Edge layer LIVE: **`graph/edges/edges.jsonl` = 22,255 cited edges** (deterministic core v1.3 → … → S104 PRECEDES + RR causal chain → S105 Bran's-fall arc → S106 +41 two Tier-A arcs [Sack-of-KL + Purple-Wedding] → S107 +26 two Tier-B arcs [B1 Red-Wedding-upstream + B2 Greyjoy→Theon-ward] → **S108 +14: B3 Ned's-downfall arc (dip-gated, Q10) — 5 causal Tier-2: `discovery MOTIVATES eddard/cersei`, `death-of-robert CAUSES arrest CAUSES execution`, `confession TRIGGERS execution`; agency on role edges (Lancel AGENT_IN + Cersei COMMANDS_IN the death; Littlefinger COMMANDS_IN the betrayal); hard-stops short of WO5K**). Query primitive **`graph-query.py --causal-chain`** (walks CAUSES/TRIGGERS/MOTIVATES both directions). Node connectivity **~71%**. **7 evidence kinds** (incl. **`derived-chronology`** Tier-3 S104; **`wiki-historical-anchor`** Tier-2): wiki-infobox 17,006 / book-pass1 ~4,010 / book-pass1-reified 897 / derived-chronology 174 / plate4-wiki-cluster 51 / wiki-historical-anchor ~29 / book-curator 11 (+ causal-curator-arc ~57). **128 edge types** (no new types since `MOTIVATES`, S105). Vocab **167**.
...
**IN FLIGHT**
- (none — S108 work complete, awaiting Matt's commit OK.) **S108 built+verified B3 (Ned's-downfall arc, the re-dip's Q10 gap) and re-dipped: Q10 partial → CORRECT and Q8 partial → CORRECT (first measure of the post-B2 graph). Arc layer now 8 correct / 1 policy-stop-short / 2 partial.** Continue prompt `2026-06-18-causal-arc-execution.md` fully consumed (B1/B2/B3 all shipped + re-dipped).

**NEXT TRACK (S108 → S109)**
- **Dip-driven, do NOT mass-mint.** Post-B3 re-dip re-ranking (`working/session-results/2026-06-19-post-b3-redip.md`): **#1 = Q7 `robb-weds-jeyne-westerling` upstream refinement** (why Robb married Jeyne — fall of the Crag / Jeyne nursing him; extends the shipped B1; 1–2 beats); #2 = Q3 Trident inbound CAUSES (`roberts-rebellion CAUSES battle-of-the-trident` or PRECEDES→CAUSES promotion; 1 edge); #3 = `execution-of-eddard-stark` downstream consequences (lower priority — the asked blame/cause facet is covered). **Before building, re-run an arc-weighted dip to confirm demand** (B3 was the richest gap and it's now closed — the remaining items are refinements). Strategy + rubric: `working/causal-arc-strategy-2026-06-18.md`. Reuse the proven machine (research subagent → orchestrator mints via `scripts/mint_*_arc.py` → index+alias rebuild → fresh-subagent verify → `--causal-chain` smoke test). **Verification (FIRM, Matt):** interpretive/causal edges checked by fresh subagents vs LOCAL cache; Matt gates at policy level, not per-edge. (**Sonnet 4.6**)
- Reusable arc-mint machine now proven 7× (RR, Bran, Sack, PW, B1, B2, B3): research subagent (dedup + quotes + edge proposal) → orchestrator trims/mints via a `scripts/mint_*_arc.py` script (backup + re-run guard) → targeted index + alias rebuild → fresh-subagent verify (causal edges `verified_by: pending` until CONFIRM) → `--causal-chain` smoke test.
...
> archive022 = S102–S103 (FILLING, 2/5 — S103 rotated out S108) · archive021 = S97–S101 (FULL) · archive020 = S92–96 · archive019 = S87–S91 · archive018 = S83(×2: /tmp-paths + Plates 0-2)–S86 · archive017 = S78–82 · archive016 = S73–77 · archive015 = S68–72
