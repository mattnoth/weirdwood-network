...
### STATUS — at a glance (verified 2026-06-19, S107)

**SHIPPED**
- Pass 1 mechanical extraction: **344/344 chapters, all 5 books** (done 2026-05-06, all Opus)
- Wiki cache local (17,945 fetched → 17,657 unique files) + Pass 2 promotion: **graph/nodes/ = 8,539** (events incl. S104 RR spark-beats + 5 S105 Bran's-fall beats + 8 S106 arc beats [4 Sack-of-KL + 4 Purple-Wedding] + **6 S107 beats [5 B1 Red-Wedding-upstream + 1 B2 Theon-ward]**; excl. `_conflicts/` staging)
- Entity + chapter indexes: **all 21 categories** (S72)
- Edge layer LIVE: **`graph/edges/edges.jsonl` = 22,241 cited edges** (deterministic core v1.3 → … → S104 PRECEDES + RR causal chain → S105 Bran's-fall arc → S106 +41 two Tier-A arcs [Sack-of-KL + Purple-Wedding] → **S107 +26: two Tier-B arcs (dip-gated) — B1 Red-Wedding-upstream (+21; two parallel chains: `catelyn-frees-jaime CAUSES karstark-murders CAUSES rickard-execution` + `robb-weds-jeyne TRIGGERS red-wedding-conspiracy CAUSES red-wedding/robb-is-killed`; conspiracy beat carries agency) + B2 Greyjoy→Theon-ward (+5; `greyjoy-rebellion CAUSES theon-greyjoy-taken-as-ward`); hard-stops short of WO5K / Theon's ACOK invasion**). Query primitive **`graph-query.py --causal-chain`** (walks CAUSES/TRIGGERS/MOTIVATES both directions). Node connectivity **~71%**. **7 evidence kinds** (incl. **`derived-chronology`** Tier-3 S104; **`wiki-historical-anchor`** Tier-2): wiki-infobox 17,006 / book-pass1 ~4,010 / book-pass1-reified 897 / derived-chronology 174 / plate4-wiki-cluster 51 / wiki-historical-anchor ~29 / book-curator 11 (+ causal-curator-arc ~38). **128 edge types** (no new types since `MOTIVATES`, S105). Vocab **167**.
...
**IN FLIGHT**
- (none — S107 work complete, awaiting Matt's commit OK.) **S107 ran the arc-weighted dip (5/10 correct, all 4 shipped arcs validated), and minted+verified the dip's clearest fumble — the B1 Red-Wedding-upstream arc (Q7).** Continue prompt `2026-06-18-causal-arc-execution.md`: Step A done (twice); Step B partially consumed (B1 shipped; B2 dip-gated).

**NEXT TRACK (S107 → S108)**
- **Re-run an arc-weighted Mode-3 dip BEFORE building B2 or any further Tier-B/C arc** (strategy's dip-driven cadence — don't mass-mint). B1 closed Q7; a fresh dip confirms what (if anything) agents still fumble. B2 (Greyjoy→Theon-as-ward) is dip-gated (Q8 was only *partial* — the WARD_OF dyad already answers the core fact). Strategy + rubric: `working/causal-arc-strategy-2026-06-18.md`; continue prompt `progress/continue-prompts/2026-06-18-causal-arc-execution.md`. **Verification (FIRM, Matt):** interpretive/causal edges checked by fresh subagents vs LOCAL cache; Matt gates at policy level, not per-edge. (**Sonnet 4.6**)
...
> archive022 = S102– (FILLING, 1/5 — S102 rotated out S107) · archive021 = S97–S101 (FULL) · archive020 = S92–96 · archive019 = S87–S91 · archive018 = S83(×2: /tmp-paths + Plates 0-2)–S86 · archive017 = S78–82 · archive016 = S73–77 · archive015 = S68–72
