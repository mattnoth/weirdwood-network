...
### RESOLVED: Events Haiku bulk — sleep lowered to 120s for the unattended phase (2026-05-28, Session 79)
- **DONE.** The S77 "lower the sleep before Matt travels" action item is resolved. Matt runs it himself in iTerm; final unattended pacing = **`STAGE4_SLEEP_BETWEEN=120`** (Matt's call S79 — finishes faster than the originally-planned 600/300; pacing is a usage-rate lever, not a cost lever, so total spend is unchanged ~$50).
- **Current launch command (stable output dir so cross-day resume works — do NOT rely on the datestamp default):**
  ```
  STAGE4_OUT=/Users/mnoth/source/asoiaf-chat/working/wiki/pass2-buckets/pass1-derived/_events-haiku-bulk \
  STAGE4_SLEEP_BETWEEN=120 STAGE4_VALIDATE_EVERY=25 \
  bash /Users/mnoth/source/asoiaf-chat/scripts/stage4-events-bulk-run.sh
  ```
- **To change the sleep again:** `touch /tmp/stage4-stop` (classifier checks the stop-file in 30s chunks during its inter-batch sleep + after each batch; stops within ~1 batch) → wait for clean exit → relaunch the SAME command with the new `STAGE4_SLEEP_BETWEEN` and the **same `STAGE4_OUT`** (`--skip-existing` resumes losslessly). Note: `setsid` is NOT available on macOS — use `nohup` or just iTerm for a persistent launch. Runbook: `working/runbooks/stage4-events-haiku-bulk.md`.
- **Known loose end (S79, non-fatal):** a recurring ~40-row acok batch fails with `JSON parse error: Expecting value: line 1 column 1 (char 0)` every run (Haiku returns non-JSON → whole-batch fail → `classify_failed`, which is NOT a skip-key, so it's re-fed each run). Rare (2/~700 batches) and non-degrading. Fix is a one-liner queued in `working/todos.md`.
- Drift safety: `--validate-every 25` hard-stops (exit 43) on schema drift / reject-rate <0.70. Completion read (precision + reject-recall) still owed (orchestrator).
